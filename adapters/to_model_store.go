...
func BistreTurnToStore(ctx context.Context, store *model.Store, source string, turn *bistre.Turn_t, game, clanNo string) (int64, int64, error) {
	now := time.Now().UTC()
...
func insertUnitMoves(ctx context.Context, store *model.Store, rxID, rfID int64, turnNo int, unitId bistre.UnitId_t, moves *bistre.Moves_t) error {
	ux := &model.UnitX{
		ReportXID: rxID,
